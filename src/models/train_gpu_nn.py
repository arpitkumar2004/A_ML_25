...
def train_mlp_gpu(X_train, y_train, X_val, y_val, epochs=1000, lr=1e-3, device='cuda'):
...
def train_multiple_models_gpu(df, feat_cols, target_col, save_models=True):
    ...
    alphas = [0.001, 0.01, 0.1, 10.0, 100.0]
    for alpha in alphas:
    ...
    mlp_model = train_mlp_gpu(X_train_scaled, y_train, X_test_scaled, y_test, epochs=1000, lr=1e-3, device=device)
    mlp_model.eval()
    with torch.no_grad():
    ...
    lgb_model = lgb.LGBMRegressor(n_estimators=500, learning_rate=0.05, max_depth=-1, device='gpu', random_state=42)
